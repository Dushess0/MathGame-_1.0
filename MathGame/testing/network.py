...
class NetWork():
    ...
    def send_sets(self):
        ...
        for enemy in self.enemies:
            self.send_message_to_enemy(enemy,"new sets"+"'"+data+"'"+self.str_addres())
    ...
    def battle_commands(self,data,message):
          ...
          elif message.startswtih("new sets"):
              sets=message[message.find("'")+1:message.find("'")]
              sets = json.loads(sets)
              i=0
              for player in self.comm.players:
                  print('iteration')
                  player.exp.set=sets[i]
                  i+=1
